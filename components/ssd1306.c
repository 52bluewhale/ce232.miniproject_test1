...
void task_ssd1306_display_text(const void *arg_text) 
{
	...
	cmd = i2c_cmd_link_create();	//command link
	i2c_master_start(cmd);	//start bit (cmd)
	i2c_master_write_byte(cmd, (OLED_I2C_ADDRESS << 1) | I2C_MASTER_WRITE, true);	//address  frame (cmd,address,ack)
	i2c_master_write_byte(cmd, OLED_CONTROL_BYTE_CMD_STREAM, true);	//read/write bit (cmd,data,n,ack)
...
/*
void task_ssd1306_set_page(int cur_page)
{
	i2c_cmd_handle_t cmd;
